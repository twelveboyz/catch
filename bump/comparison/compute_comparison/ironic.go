...
func IronicComparison(excel get.ComputeResource, consoleIronics []*ironic.Ironic) {
	var mark = false
	logrus.Debugf("------------------------------ Row:%s ------------------------------", excel.Row)
	for _, consoleIronic := range consoleIronics {

		if excel.ResourceID == consoleIronic.Id {
			mark = true
			iinternal.IfFieldComparison("标签1", excel.Row, strings.TrimLeft(excel.Tag1, "_"), consoleIronic.Tag1)

			iinternal.IfFieldComparison("标签X", excel.Row, strings.TrimLeft(excel.TagX, "_"), consoleIronic.TagX)

			iinternal.IfFieldComparison("标签2", excel.Row, excel.Tag2, consoleIronic.Tag2)

			iinternal.IfFieldComparison(" ID ", excel.Row, excel.ResourceID, consoleIronic.Id)

			iinternal.IfFieldComparison("Name", excel.Row, excel.ResourceName, consoleIronic.Name)

			iinternal.IfFieldComparison("规格", excel.Row, excel.Specification, consoleIronic.SpecsName)

			iinternal.IfFieldComparison("镜像", excel.Row, excel.Image, consoleIronic.ImageName)

			iinternal.IfFieldComparison("CPU", excel.Row, excel.CPU, consoleIronic.VCpu)

			iinternal.IfFieldComparison("Memory", excel.Row, excel.Memory, consoleIronic.VMemory)

			iinternal.IfFieldComparison("系统盘规格", excel.Row, excel.SystemDiskSpec, consoleIronic.BootVolumeType)

			iinternal.IfFieldComparison("系统盘大小", excel.Row, excel.SystemDiskSize, consoleIronic.VDisk)
			...
			for i, eNicName := range []string{excel.NIC1.Name, excel.NIC2.Name, excel.NIC3.Name} {
				...
				var nicMark bool
				for _, n := range consoleIronic.NIC {
					//NIC1---------------
					if eNicName == n.Name && i == 0 {
						...
						for _, esg := range excelSgs {
							for _, sg := range n.SecurityGroup {
								if esg == sg.Name {
									iinternal.IfFieldComparison("安全组", excel.Row, esg, sg.Name)
								}
							}
						}
				...
				if !nicMark {
					var nicList []string
					for _, nic := range consoleIronic.NIC {
						nicList = append(nicList, nic.Name)
					}
					logrus.Warnf("Row:%s 网卡%d excel: %s,未匹配到控制台网卡名称:%v,", excel.Row, i+1, eNicName, nicList)
				}
			}

			iinternal.IfFieldComparison("资源池", excel.Row, excel.Node, mapping.PoolCodeToNameConvert(consoleIronic.Pool))

			iinternal.IfFieldComparison("可用区", excel.Row, excel.AvailabilityZone, consoleIronic.Region)
		}
	}
	if !mark {
